...
class BowBase extends BowItem implements Vanishable {
    public BowBase(Item.Settings settings) {
        super(new Item.Settings().group(RubinMod.ITEM_GROUP).maxCount(1).maxDamage(1400));
    }
        ...
        if (user instanceof PlayerEntity) {
            ...
            if (!itemStack.isEmpty() || bl) {
                ...
                if (!((double)f < 0.1D)) {
                    ...
                    if (!world.isClient) {
                        assert (itemStack.getItem() instanceof ArrowItem ? itemStack.getItem() : Items.ARROW) instanceof ArrowItem;
                        ...
                        if (j > 0) {
                            persistentProjectileEntity.setDamage(persistentProjectileEntity.getDamage() + (double)j * 0.8D + 0.8D);
                        }
